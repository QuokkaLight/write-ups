...
In the `README` file we have the following snippet of asm code.
...
The flag is in `$t0`. `$t2` holds the `$t0`th character of the flag which is then loaded in `$a0`. After that, `$a0` is XORed with `$t1` which is a variable that is incrementing every loop.

Thus, in order to decode `IVyN5U3X)ZUMYCs` we need to XOR the `i`th character with the number `i`.

We can get the flag with the following script.
...
```bash
rainbowlyte@ns38534:/tmp$ python decoder.py
IW{M1P5_!S_FUN}
```